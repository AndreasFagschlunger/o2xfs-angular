...
import { ServiceClass, XfsManager } from 'xfs';
import { Statement } from '@angular/compiler';

@Injectable()
export class CdmGuard implements CanActivateChild {

    constructor(private menuService: MainMenuService, private xfsManager: XfsManager, private router: Router) { }
    ...
    canActivateChild(childRoute: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
        ...
        if (!result) {
            let cashDispensers = this.xfsManager.getLogicalServices().filter(e => ServiceClass.CDM === e.serviceClass);
